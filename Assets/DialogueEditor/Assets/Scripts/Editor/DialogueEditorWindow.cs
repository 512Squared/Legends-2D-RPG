...
namespace DialogueEditor
{
    public class DialogueEditorWindow : EditorWindow
    {
        ...
        public class SelectableUINode : SelectableUI
        {
            ...
            public override eType Type => eType.Node;
        ...
        public class SelectableUIConnection : SelectableUI
        {
            ...
            public override eType Type => eType.Connection;
        ...
        // Private variables:     
        private NPCConversation
            CurrentAsset; // The Conversation scriptable object that is currently being viewed/edited
        ...
        public void RecreateUI(EditableConversation conversation)
        {
            ...
            for (int i = 0; i < allNodes.Count; i++)
            {
                ...
                {
                    UISpeechNode uiNode = new UISpeechNode(thisNode,
                ...
                {
                    UIOptionNode uiNode = new UIOptionNode(thisNode,
                        new Vector2(thisNode.EditorInfo.xPos, thisNode.EditorInfo.yPos));
                    uiNodes.Add(uiNode);
                }
            }
        ...
        private void DrawMessageDuringPlay()
        {
            ...
            Rect textRect = new Rect(centerX - (textDimensions.x / 2), centerY - (textDimensions.y / 2),
                textDimensions.x, textDimensions.y);
        ...
        private void DrawPanel()
        {
            ...
            if (CurrentlySelectedObject == null)
            {
                ...
                EditorGUILayout.LabelField("Default Font:", GUILayout.MinWidth(labelWidth),
                    GUILayout.MaxWidth(labelWidth));
                CurrentAsset.DefaultFont = (TMPro.TMP_FontAsset)EditorGUILayout.ObjectField(CurrentAsset.DefaultFont,
                    typeof(TMPro.TMP_FontAsset), false, GUILayout.MaxWidth(fieldWidth));
                EditorGUILayout.EndHorizontal();
                ...
                EditorGUILayout.LabelField("'Continue' font:", GUILayout.MinWidth(labelWidth),
                    GUILayout.MaxWidth(labelWidth));
                CurrentAsset.ContinueFont = (TMPro.TMP_FontAsset)EditorGUILayout.ObjectField(CurrentAsset.ContinueFont,
                    typeof(TMPro.TMP_FontAsset), false, GUILayout.MaxWidth(fieldWidth));
                EditorGUILayout.EndHorizontal();
                ...
                EditorGUILayout.LabelField("'End' font:", GUILayout.MinWidth(labelWidth),
                    GUILayout.MaxWidth(labelWidth));
                CurrentAsset.EndConversationFont = (TMPro.TMP_FontAsset)EditorGUILayout.ObjectField(
                    CurrentAsset.EndConversationFont, typeof(TMPro.TMP_FontAsset), false,
            ...
            else
            {
                ...
                if (CurrentlySelectedObject.Type == SelectableUI.eType.Node)
                {
                    ...
                    if (selectedNode is UISpeechNode)
                    {
                        ...
                        node.TMPFont =
                            (TMPro.TMP_FontAsset)EditorGUILayout.ObjectField(node.TMPFont, typeof(TMPro.TMP_FontAsset),
                                false);
                        EditorGUILayout.Space();
                ...
                else if (CurrentlySelectedObject.Type == SelectableUI.eType.Connection)
                {
                    ...
                    // Button
                    {
                        ...
                        if (GUILayout.Button("Add condition"))
                        {
                            ...
                            for (int i = 0; i < CurrentAsset.ParameterList.Count; i++)
                            {
                                ...
                                if (CurrentAsset.ParameterList[i].ParameterType == EditableParameter.eParamType.Int)
                                {
                                ...
                                else if (CurrentAsset.ParameterList[i].ParameterType ==
                    ...
                    for (int i = 0; i < connection.Conditions.Count; i++)
                    {
                        ...
                        else if (connection.Conditions[i].ConditionType ==
                ...
                // Button
                {
                    ...
                    if (GUILayout.Button("Add Parameter Action"))
                    {
                        ...
                        for (int i = 0; i < CurrentAsset.ParameterList.Count; i++)
                        {
                            ...
                            else if (CurrentAsset.ParameterList[i].ParameterType == EditableParameter.eParamType.Bool)
                            {
                                EditableBoolParameter boolParam =
                                    CurrentAsset.ParameterList[i] as EditableBoolParameter;
                                rightClickMenu.AddItem(new GUIContent(boolParam.ParameterName), false, delegate
            ...
            if (m_connectionDeleteParent != null && m_connectionDeleteChild != null)
            {
                ...
                for (int i = 0; i < m_connectionDeleteParent.Connections.Count; i++)
                {
                    ...
                    if (connection is EditableSpeechConnection &&
                        (connection as EditableSpeechConnection).Speech == m_connectionDeleteChild)
                    {
                        m_connectionDeleteParent.Connections.RemoveAt(i);
                        i--;
                    }
                    else if (connection is EditableOptionConnection &&
                             (connection as EditableOptionConnection).Option == m_connectionDeleteChild)
                    {
                        m_connectionDeleteParent.Connections.RemoveAt(i);
                        i--;
                    }
        ...
        private void Recenter()
        {
            ...
            Vector2 target = new Vector2((position.width / 2) - (UISpeechNode.Width / 2) - (panelWidth / 2),
                TOOLBAR_HEIGHT + 5);
