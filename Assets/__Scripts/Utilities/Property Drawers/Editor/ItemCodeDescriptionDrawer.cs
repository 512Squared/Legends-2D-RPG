...
public class ItemCodeDescriptionDrawer : PropertyDrawer
{
    ...
    public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
    {
        ...
        if (property.propertyType == SerializedPropertyType.Integer)
        {
            ...
            int newValue =
                EditorGUI.IntField(new Rect(position.x, position.y, position.width, position.height - ImageHeight - 50),
                    label, property.intValue);

            // Draw item name
            EditorGUI.LabelField(
                new Rect(position.x, position.y + 20, position.width, position.height - ImageHeight - 50), "Item Name",
                GetItemDescription(property.intValue));
            // Draw Item image
            GUI.DrawTexture(new Rect(position.x, position.y + 40, position.width, position.height - 50),
                GetItemImage(property.intValue).texture, ScaleMode.ScaleToFit);
    ...
    private static string GetItemDescription(int itemCode)
    {
        ...
        if (itemCode > itemDetailsList.Count + 1001 || itemCode < 1001)
        {
            return "ITEM CODE OUT OF RANGE - PLEASE try again!";
        }

        ItemDetails itemDetail = itemDetailsList.Find(x => x.itemCode == itemCode);
        if (itemDetail == null)
        {
            return "ERROR - is the item code in SO_itemsList correct?";
        }
    ...
    private static Sprite GetItemImage(int itemCode)
    {
        ...
        List<ItemDetails> itemDetailsList = so_itemList!.itemsDetails;
        if (itemCode > itemDetailsList.Count + 1001 || itemCode <= 1000)
        {
            return error;
        }
